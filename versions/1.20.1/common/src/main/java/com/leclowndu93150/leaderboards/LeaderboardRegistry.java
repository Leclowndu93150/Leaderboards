    ...
    public static void register() {
        ...
        put("last_seen", new Leaderboard(
                ...
                player -> {
                    ...
                    long lastSeen = PlayerDataTracker.get(player.server()).getLastSeen(player.uuid());
                    if (lastSeen == 0L) {
                ...
                Comparator.comparingLong(p -> {
                    ...
                    return p.server().overworld().getGameTime() - PlayerDataTracker.get(p.server()).getLastSeen(p.uuid());
                }),
                player -> player.isOnline() || PlayerDataTracker.get(player.server()).getLastSeen(player.uuid()) != 0L));

        VanillaStatsRegistry.register();
    }

    private static ResourceLocation id(String path) {
        return new ResourceLocation(Leaderboards.MODID, path);
    }
